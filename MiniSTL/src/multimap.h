...
struct my_multimap{
    ...
    //range构造函数
    template<class iter>
    my_multimap(iter first, iter last)
    :mytree()
    {
        mytree.rb_insert_range_multi(first,last);
    }

    //拷贝构造
    my_multimap(const my_multimap& other): mytree(other.mytree) {}

    //移动构造
    my_multimap(my_multimap&& other): mytree(mystl::move(other.mytree)) {}

    //initializer list构造
    my_multimap(std::initializer_list<my_valType> ilist)
    : mytree()
    {
        mytree.rb_insert_range_multi(ilist.begin(), ilist.end());
    }

    //拷贝赋值
    my_multimap& operator=(const my_multimap& other){
        mytree=other.mytree;
        return *this;
    }
    //移动赋值
    my_multimap& operator=(my_multimap&& other){
        mytree=mystl::move(other.mytree);
        return *this;
    }
    //initialize_list赋值
    my_multimap& operator=(std::initializer_list<my_valType> ilist){
        mytree.clear();
        mytree.rb_insert_range_multi(ilist.begin(),ilist.end());
        return *this;
    }
    ...
    //swap
    void swap(my_multimap& other) noexcept { mytree.swap(other.mytree); }
    ...
    size_type count(const key_type& val) { return mytree.count_multi(val); }
    ...
    std::pair<iterator,iterator> equal_range(const key_type& val) { return mytree.equal_range_multi(val); }
    std::pair<const_iterator,const_iterator> equal_range(const key_type& val) const { return mytree.equal_range_multi(val); }

    //插入删除
    //插入值为val的节点     拷贝
    iterator insert(const my_valType& val) { return mytree.emplace_multi(val); }
    //插入值为val的节点     移动
    iterator insert(my_valType&& val) { return mytree.emplace_multi(mystl::move(val)); }
    //给定插入位置在pos附近     拷贝
    iterator insert(iterator pos, const my_valType& val) { return mytree.emplace_hint_multi(pos,val); }
    //给定插入位置在pos附近     移动
    iterator insert(iterator pos, my_valType&& val) { return mytree.emplace_hint_multi(pos,mystl::move(val)); }
    //给定range插入
    template<class iter>
    void insert(iter first, iter last) { mytree.rb_insert_range_multi(first,last); }
    //initializer_list插入
    void insert(std::initializer_list<my_valType> ilist) { mytree.rb_insert_range_multi(ilist.begin(),ilist.end()); }

    //删除迭代器所指位置节点
    void erase(const_iterator pos) { mytree.rb_erase(pos.node, mytree.root()); }
    //删除容器中键值为val的节点
    size_type erase(const key_type& val) { return mytree.erase_val_multi(val); }
    //删除range中迭代器所指节点
