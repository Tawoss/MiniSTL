...
struct my_multiset{
    ...
    //range构造函数
    template<class iter>
    my_multiset(iter first, iter last)
    :mytree()
    {
    ...
    //拷贝构造
    my_multiset(const my_multiset& other): mytree(other.mytree) {}

    //移动构造
    my_multiset(my_multiset&& other): mytree(mystl::move(other.mytree)) {}

    //initializer list构造
    my_multiset(std::initializer_list<my_valType> ilist)
    : mytree()
    {
    ...
    //拷贝赋值
    my_multiset& operator=(const my_multiset& other){
        mytree=other.mytree;
        return *this;
    }
    //移动赋值
    my_multiset& operator=(my_multiset&& other){
    ...
    //swap
    void swap(my_multiset& other) noexcept { mytree.swap(other.mytree); }
    ...
    size_type count(const my_valType& val) { return mytree.count_multi(val); }
    ...
    std::pair<iterator,iterator> equal_range(const my_valType& val) { return mytree.equal_range_multi(val); }
    std::pair<const_iterator,const_iterator> equal_range(const my_valType& val) const { return mytree.equal_range_multi(val); }
    ...
    //删除容器中值为val的节点
    size_type erase(const my_valType& val) { return mytree.erase_val_multi(val); }
    //删除range中迭代器所指节点
