...
struct myunordered_multiset{
    //定义hashtable
    typedef my_hashtable<Key,Hash,KeyEqual>    myhash_table;
    //使用 hashtable 的型别
    ...
    //默认构造
    myunordered_multiset():my_ht(100,Hash(), KeyEqual()){}
    //传入bucketcount, Hash函数, KeyEqual函数构造
    myunordered_multiset(size_type bucket_count, const Hash& hash=Hash(), const KeyEqual& equal=KeyEqual()):my_ht(bucket_count,hash,equal){}
    //传入range, bucketcount, Hash函数, KeyEqual函数构造
    template <class iter>
    myunordered_multiset(iter first, iter last, const size_type bucket_count = 100, const Hash& hash = Hash(), const KeyEqual& equal = KeyEqual())
    ...
    myunordered_multiset(std::initializer_list<my_valType> ilist, const size_type bucket_count = 100, const Hash& hash = Hash(), const KeyEqual& equal = KeyEqual())
    ...
    //拷贝构造
    myunordered_multiset(const myunordered_multiset& other):my_ht(other.my_ht) {}
    //移动构造
    myunordered_multiset(myunordered_multiset&& other):my_ht(mystl::move(other.my_ht)) {}

    //拷贝赋值
    myunordered_multiset& operator=(const myunordered_multiset& other){
        my_ht=other.my_ht;
        return *this;
    }
    //移动赋值
    myunordered_multiset& operator=(myunordered_multiset&& other){
        my_ht=mystl::move(other.my_ht);
        return *this;
    }
    //初始化列表赋值
    myunordered_multiset& operator=(std::initializer_list<my_valType> ilist){
        //清空原来的容器
        my_ht.clear();
        //增加hash bucket数量
    ...
    //emplace
    template<class ...Args>
    std::pair<iterator, bool>  emplace(Args&& ...args) { return my_ht.emplace_multi(mystl::forward<Args>(args)...); }
    template<class ...Args>
    std::pair<iterator, bool>  emplace_hint(const_iterator pos,Args&& ...args) { return my_ht.emplace_multi_hint(pos,mystl::forward<Args>(args)...); }

    //insert
    std::pair<iterator,bool>    insert(const my_valType& val) { return my_ht.insert_multi(val); }
    std::pair<iterator,bool>    insert(my_valType&& val) { return my_ht.emplace_multi(mystl::move(val)); }
