...
struct myunordered_set{
    ...
    //默认构造
    myunordered_set():my_ht(100,Hash(), KeyEqual()){}
    //传入bucketcount, Hash函数, KeyEqual函数构造
    myunordered_set(size_type bucket_count, const Hash& hash=Hash(), const KeyEqual& equal=KeyEqual()):my_ht(bucket_count,hash,equal){}
    //传入range, bucketcount, Hash函数, KeyEqual函数构造
    template <class iter>
    myunordered_set(iter first, iter last, const size_type bucket_count = 100, const Hash& hash = Hash(), const KeyEqual& equal = KeyEqual())
    ...
    myunordered_set(std::initializer_list<my_valType> ilist, const size_type bucket_count = 100, const Hash& hash = Hash(), const KeyEqual& equal = KeyEqual())
    ...
    //拷贝构造
    myunordered_set(const myunordered_set& other):my_ht(other.my_ht) {}
    //移动构造
    myunordered_set(myunordered_set&& other):my_ht(mystl::move(other.my_ht)) {}

    //拷贝赋值
    myunordered_set& operator=(const myunordered_set& other){
        my_ht=other.my_ht;
        return *this;
    }
    //移动赋值
    myunordered_set& operator=(myunordered_set&& other){
        my_ht=mystl::move(other.my_ht);
        return *this;
    }
    //初始化列表赋值
    myunordered_set& operator=(std::initializer_list<my_valType> ilist){
        //清空原来的容器
        my_ht.clear();
        //增加hash bucket数量
